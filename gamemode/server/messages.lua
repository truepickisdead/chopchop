...
hook.Add("player_connect", "ShowConnect", function( data )
	chopchop.chat:Send(
		player.GetAll(),
		chopchop.settings.colors.chatMsgDefault, translate.make( "msgPlyConnect", data["name"] )
	)
end)
...
hook.Add("player_disconnect", "ShowDisconnect", function( data )
	chopchop.chat:Send(
		player.GetAll(),
		chopchop.settings.colors.chatMsgDefault, translate.make( "msgPlyDisconnect", data["name"] )
	)
end)
